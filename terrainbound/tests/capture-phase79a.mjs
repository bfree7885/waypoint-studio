#!/usr/bin/env node
/**
 * Phase 7.9A Cedar Hollow puzzle captures.
 * Writes PNGs to tests/evidence/phase79a/.
 */
...
const OUT = path.join(ROOT, "tests/evidence/phase79a");
const PORT = Number(process.env.TB_HTTP_PORT || 8093);
const GAME = `http://127.0.0.1:${PORT}/terrainbound/?field=1&v=p79a`;
const DEBUG_PORT = Number(process.env.TB_CDP_PORT || 9349);
const USER_DATA = `/tmp/tb-p79a-chrome-${process.pid}`;
...
function launchChrome() {
  const logFd = openSync(`/tmp/tb-p79a-chrome-${process.pid}.log`, "w");
  const child = spawn(
    "google-chrome",
    [
      "--headless=new",
...
async function main() {
  ...
  try {
    ...
    for (const vp of VIEWPORTS) {
      ...
      await sleep(220);
      await shot(page, `tb-p79a-${vp.name}-tablet.png`);
