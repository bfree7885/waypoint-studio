#!/usr/bin/env node
/**
 * Phase 7.9L production Summit captures (no field-test chrome).
 * Writes PNGs to tests/evidence/phase79l/.
 */
...
const OUT = path.join(ROOT, "tests/evidence/phase79l");
const PORT = Number(process.env.TB_HTTP_PORT || 8091);
const GAME_AI = `http://127.0.0.1:${PORT}/terrainbound/?field=1&summit=local&v=p79l`;
const GAME_LOCAL = `http://127.0.0.1:${PORT}/terrainbound/?field=1&summit=local&v=p79l`;
const GAME_NORMAL = `http://127.0.0.1:${PORT}/terrainbound/?field=1&v=p79l`;
const DEBUG_PORT = Number(process.env.TB_CDP_PORT || 9361);
const USER_DATA = `/tmp/tb-p79l-chrome-${process.pid}`;
...
function launchChrome() {
  const logFd = openSync(`/tmp/tb-p79l-chrome-${process.pid}.log`, "w");
  const child = spawn(
    "google-chrome",
    [
      "--headless=new",
...
const MEASURE = `(() => {
  ...
  const toggle = box(document.querySelector("#summit-toggle"));
  const toggleFace = box(document.querySelector(".summit-toggle-face"));
  ...
  const portrait = box(document.querySelector("#summit:not([hidden]) #summit-portrait"));
  const role = document.querySelector("#summit-role")?.textContent || "";
  const quick = [...document.querySelectorAll("#summit-quick button")].map((btn) => {
    const r = btn.getBoundingClientRect();
    return { label: btn.textContent, w: r.width, h: r.height, y: r.y };
  });
  return {
    ...
    lead,
    role,
    expression: document.querySelector("#summit-portrait")?.dataset.expression || "",
    portraitW: portrait?.w || 0,
    portraitH: portrait?.h || 0,
    portraitVisible: Boolean(portrait && portrait.w >= 48 && portrait.h >= 48),
    toggleVisible: Boolean(toggle && toggle.w >= 44 && toggle.h >= 36),
    toggleFaceVisible: Boolean(toggleFace && toggleFace.w >= 20 && toggleFace.h >= 20),
    toggleLabel: document.querySelector("#summit-toggle")?.textContent?.replace(/\s+/g, " ").trim() || "",
...
async function enterGame(page, url) {
  ...
  await waitFor(() => page.evaluate(`Boolean(document.querySelector("#dialogue")?.hidden)`), 2000).catch(() => null);
  await sleep(150);
}
...
async function main() {
  ...
  try {
    ...
    for (const vp of VIEWPORTS) {
      ...
      await shot(page, `tb-p79l-${vp.name}-closed.png`);
      const hudClosed = await page.evaluate(MEASURE);
      ...
      await shot(page, `tb-p79l-${vp.name}-neutral.png`);

      await page.evaluate(`window.TB.askSummit({ question: "how do i play" })`, true);
      const help = await page.evaluate(MEASURE);
      await shot(page, `tb-p79l-${vp.name}-gamehelp.png`);

      await page.evaluate(`window.TB.askSummit({ question: "are you bigfoot" })`, true);
      await shot(page, `tb-p79l-${vp.name}-character.png`);
      ...
      await page.evaluate(`window.TB.askSummit({ question: "explain it easier" })`, true);
      const thread = await page.evaluate(MEASURE);
      await shot(page, `tb-p79l-${vp.name}-long.png`);

      await page.evaluate(`document.querySelector("#summit-ask")?.focus()`);
      const keyboard = await page.evaluate(MEASURE);
      await shot(page, `tb-p79l-${vp.name}-keyboard.png`);

      await page.evaluate(`window.__closeAsk = window.TB.askSummit({ question: "what should I notice?" })`);
      await sleep(120);
      ...
      await shot(page, `tb-p79l-${vp.name}-fallback.png`);
      ...
      await shot(page, `tb-p79l-${vp.name}-normal.png`);
      ...
      console.log(
        vp.name,
        JSON.stringify({
          toggle: hudClosed.toggleVisible,
          toggleFace: hudClosed.toggleFaceVisible,
          toggleLabel: hudClosed.toggleLabel,
          portrait: opened.portraitVisible,
          portraitW: opened.portraitW,
          expressionOpen: opened.expression,
          expressionThink: pending.expression,
          helpHasLoop: /Field Tablet|Wren decides/i.test(help.log),
          cannedOrient: /You are trying to say/i.test(help.log),
          ...
          overflow: arrived.horizontalOverflow || thread.horizontalOverflow || marked.horizontalOverflow || help.horizontalOverflow
        })
      );
    }
