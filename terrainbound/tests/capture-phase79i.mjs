#!/usr/bin/env node
/**
 * Phase 7.9I Summit real-model captures.
 * Writes PNGs to tests/evidence/phase79i/.
...
const GAME_AI = `http://127.0.0.1:${PORT}/terrainbound/?field=1&summit=fieldtest&v=p79i`;
const GAME_LOCAL = `http://127.0.0.1:${PORT}/terrainbound/?field=1&summit=local&v=p79i`;
const GAME_NORMAL = `http://127.0.0.1:${PORT}/terrainbound/?field=1&v=p79i`;
const DEBUG_PORT = Number(process.env.TB_CDP_PORT || 9355);
const USER_DATA = `/tmp/tb-p79i-chrome-${process.pid}`;
...
function launchChrome() {
  const logFd = openSync(`/tmp/tb-p79i-chrome-${process.pid}.log`, "w");
  const child = spawn(
    "google-chrome",
    [
      "--headless=new",
...
async function main() {
  ...
  try {
    ...
    for (const vp of VIEWPORTS) {
      ...
      await shot(page, `tb-p79i-${vp.name}-open.png`);
      ...
      await shot(page, `tb-p79i-${vp.name}-fallback.png`);
      ...
      await shot(page, `tb-p79i-${vp.name}-normal.png`);
