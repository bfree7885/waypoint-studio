#!/usr/bin/env node
/**
 * Phase 7.9K Summit character captures.
 * Writes PNGs to tests/evidence/phase79k/.
...
const OUT = path.join(ROOT, "tests/evidence/phase79k");
const PORT = Number(process.env.TB_HTTP_PORT || 8099);
const GAME_AI = `http://127.0.0.1:${PORT}/terrainbound/?field=1&summit=fieldtest&v=p79k`;
const GAME_LOCAL = `http://127.0.0.1:${PORT}/terrainbound/?field=1&summit=local&v=p79k`;
const GAME_NORMAL = `http://127.0.0.1:${PORT}/terrainbound/?field=1&v=p79k`;
const DEBUG_PORT = Number(process.env.TB_CDP_PORT || 9355);
const USER_DATA = `/tmp/tb-p79k-chrome-${process.pid}`;
...
function launchChrome() {
  const logFd = openSync(`/tmp/tb-p79k-chrome-${process.pid}.log`, "w");
  const child = spawn(
    "google-chrome",
    [
      "--headless=new",
...
async function main() {
  ...
  try {
    ...
    for (const vp of VIEWPORTS) {
      ...
      await shot(page, `tb-p79k-${vp.name}-closed.png`);
      ...
      await shot(page, `tb-p79k-${vp.name}-fallback.png`);
      ...
      await shot(page, `tb-p79k-${vp.name}-normal.png`);
