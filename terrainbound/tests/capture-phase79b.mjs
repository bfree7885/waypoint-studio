#!/usr/bin/env node
/**
 * Phase 7.9B Cedar Hollow game-feel captures.
 * Writes PNGs to tests/evidence/phase79b/.
 */
...
const OUT = path.join(ROOT, "tests/evidence/phase79b");
const PORT = Number(process.env.TB_HTTP_PORT || 8094);
const GAME = `http://127.0.0.1:${PORT}/terrainbound/?field=1&v=p79b`;
const DEBUG_PORT = Number(process.env.TB_CDP_PORT || 9350);
const USER_DATA = `/tmp/tb-p79b-chrome-${process.pid}`;
...
function launchChrome() {
  const logFd = openSync(`/tmp/tb-p79b-chrome-${process.pid}.log`, "w");
  const child = spawn(
    "google-chrome",
    [
      "--headless=new",
...
const MEASURE = `(() => {
  ...
  return {
    ...
    atlasWidth: cr?.width || 0,
    atlasHeight: cr?.height || 0,
    stacked: getComputedStyle(document.querySelector(".atlas-body")).flexDirection === "column",
    hudOverlap: overlap(prompt, mapBtn) || overlap(prompt, journalBtn) || overlap(mapBtn, journalBtn),
    prompt,
    mapBtn,
    journalBtn
  };
...
async function main() {
  ...
  try {
    ...
    for (const vp of VIEWPORTS) {
      ...
      await sleep(400);
      await shot(page, `tb-p79b-${vp.name}-gameplay.png`);
      const playMeasure = await page.evaluate(MEASURE);
      await page.evaluate(`document.querySelector("#map-toggle")?.click()`);
      await sleep(400);
      await shot(page, `tb-p79b-${vp.name}-atlas.png`);
      const atlasMeasure = await page.evaluate(MEASURE);
      await page.evaluate(`document.querySelector("#atlas-close")?.click()`);
      await sleep(120);
      await page.evaluate(`window.TB.inspectFeature("westface-slope")`);
      await sleep(220);
      await page.evaluate(`document.querySelector("#dialogue:not([hidden]) #dialogue-actions button")?.click()`);
      await sleep(180);
      await page.evaluate(`document.querySelector("#dialogue:not([hidden]) #dialogue-actions button")?.click()`);
      await sleep(220);
      await shot(page, `tb-p79b-${vp.name}-inquiry.png`);
      await page.evaluate(DISMISS, true);
      await page.evaluate(`window.TB.openSystems()`);
      await sleep(280);
      await shot(page, `tb-p79b-${vp.name}-systems.png`);
      await page.evaluate(`document.querySelector("#systems-close")?.click()`);
      await sleep(120);
      await page.evaluate(SEED);
      await page.evaluate(`window.TB.openAar()`);
      await sleep(280);
      await shot(page, `tb-p79b-${vp.name}-aar.png`);
      await page.evaluate(`document.querySelector("#aar-close")?.click()`);
      await page.evaluate(`window.TB.openJournal(); document.querySelector("#tab-evidence-btn")?.click()`);
      await sleep(220);
      await shot(page, `tb-p79b-${vp.name}-tablet.png`);
      await page.evaluate(`document.querySelector("#journal-close")?.click()`);
      report.push({ viewport: vp.name, play: playMeasure, atlas: atlasMeasure });
      console.log(vp.name, JSON.stringify({ play: playMeasure, atlas: atlasMeasure }));
    }
