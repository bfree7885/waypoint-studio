#!/usr/bin/env node
/**
 * Phase 7.8C mobile viewport captures + layout measurements.
 * Writes PNGs to tests/evidence/phase78c/.
 */
...
const OUT = path.join(ROOT, "tests/evidence/phase78c");
const PORT = Number(process.env.TB_HTTP_PORT || 8092);
const GAME = `http://127.0.0.1:${PORT}/terrainbound/?field=1&v=p78c`;
const DEBUG_PORT = Number(process.env.TB_CDP_PORT || 9348);
const USER_DATA = `/tmp/tb-p78c-chrome-${process.pid}`;
...
function launchChrome() {
  const logFd = openSync(`/tmp/tb-p78c-chrome-${process.pid}.log`, "w");
  const child = spawn(
    "google-chrome",
    [
      "--headless=new",
...
async function main() {
  ...
  try {
    ...
    for (const vp of VIEWPORTS) {
      ...
      await sleep(220);
      await shot(page, `tb-p78c-${vp.name}-fieldnote.png`);
      await page.evaluate(DISMISS, true);
