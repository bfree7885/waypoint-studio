#!/usr/bin/env node
/**
 * Phase 7.9H Summit real-model captures.
 * Writes PNGs to tests/evidence/phase79h/.
...
const OUT = path.join(ROOT, "tests/evidence/phase79h");
const PORT = Number(process.env.TB_HTTP_PORT || 8099);
const GAME_AI = `http://127.0.0.1:${PORT}/terrainbound/?field=1&summit=ai&v=p79h`;
const GAME_LOCAL = `http://127.0.0.1:${PORT}/terrainbound/?field=1&summit=local&v=p79h`;
const DEBUG_PORT = Number(process.env.TB_CDP_PORT || 9355);
const USER_DATA = `/tmp/tb-p79h-chrome-${process.pid}`;
...
function launchChrome() {
  const logFd = openSync(`/tmp/tb-p79h-chrome-${process.pid}.log`, "w");
  const child = spawn(
    "google-chrome",
    [
      "--headless=new",
...
async function main() {
  ...
  try {
    ...
    for (const vp of VIEWPORTS) {
      ...
      await shot(page, `tb-p79h-${vp.name}-open.png`);
      ...
      await shot(page, `tb-p79h-${vp.name}-pending.png`);
      await page.evaluate(`window.__pendingAsk`, true);
      const arrived = await page.evaluate(MEASURE);
      await shot(page, `tb-p79h-${vp.name}-arrived.png`);
      ...
      await shot(page, `tb-p79h-${vp.name}-fallback.png`);
