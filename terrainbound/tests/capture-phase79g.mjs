#!/usr/bin/env node
/**
 * Phase 7.9G Summit real-model captures.
 * Writes PNGs to tests/evidence/phase79g/.
 * Use ?summit=ai with a running loopback proxy for the live path.
 */
...
const OUT = path.join(ROOT, "tests/evidence/phase79g");
const PORT = Number(process.env.TB_HTTP_PORT || 8099);
const GAME_AI = `http://127.0.0.1:${PORT}/terrainbound/?field=1&summit=ai&v=p79g`;
const GAME_LOCAL = `http://127.0.0.1:${PORT}/terrainbound/?field=1&summit=local&v=p79g`;
const DEBUG_PORT = Number(process.env.TB_CDP_PORT || 9355);
const USER_DATA = `/tmp/tb-p79g-chrome-${process.pid}`;
...
function launchChrome() {
  const logFd = openSync(`/tmp/tb-p79g-chrome-${process.pid}.log`, "w");
  const child = spawn(
    "google-chrome",
    [
      "--headless=new",
...
async function main() {
  ...
  try {
    ...
    for (const vp of VIEWPORTS) {
      ...
      await shot(page, `tb-p79g-${vp.name}-open.png`);
      ...
      await shot(page, `tb-p79g-${vp.name}-pending.png`);
      await page.evaluate(`window.__pendingAsk`, true);
      const arrived = await page.evaluate(MEASURE);
      await shot(page, `tb-p79g-${vp.name}-arrived.png`);
      ...
      await shot(page, `tb-p79g-${vp.name}-fallback.png`);
      ...
      console.log(
        ...
        JSON.stringify({
          ...
          overflow: arrived.horizontalOverflow || thread.horizontalOverflow
        })
      );
    }
