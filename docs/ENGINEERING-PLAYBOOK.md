...
### 2026-08-08 — SignalTerrain Cyber production reality

**Branch:** `feature/signalterrain-production-reality`  
**Integrated:** `feature/signalterrain-live-cyber-intelligence` @ `3d55f23`

1. **Workspace must never catch-fallback to teaching samples.** Missing
   `data/cyber/graph.json` → honest error; samples only with `?teaching=1`.
2. **Nav labels are production truth.** “Adaptive Defense” / daily brief must
   point at `live.html#adaptive` / `#brief`, not sample advisor/brief pages.
3. **Sample-backed mounts need teaching gates** (explorer, knowledge, advisor):
   live graph by default; samples stay on `teaching.html` / `?teaching=1`.
4. **Regression gate:** `automation/test-signalterrain-production-reality.mjs`
   blocks silent sample substitution and nav promotion of fake Adaptive Defense.
5. **Side Trails landing should deep-link live cyber**, not only the labeled
   dashboard mockup.
