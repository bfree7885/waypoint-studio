...
- Do not run `publish-shed-hunting-host.mjs` for a metadata-only republish: it replaces the live host tree. Prefer an existing `sheds-site` checkout, keep `CNAME`, `--ff-only` pull, no force-push. Tag `legacy-terrain-intelligence-2026-03-10` is an **immutable** Terrain Intelligence → Shed Hunting cutover marker — the publisher does not move it. Each publish prints the previous sheds-site HEAD SHA; that SHA is the immediate rollback reference (manual operator action).
