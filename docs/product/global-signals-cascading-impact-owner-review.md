...
**Base:** `feature/global-signals-relationship-engine` (`aa408fa`)  
...
**expected timeframe**. Language stays estimated — never certain. Examples are
schematic literacy patterns (tariffs→consumers; conflict→inflation pressure), not
live forecasts. No application runtime in this branch.
...
4. Reuse SignalTerrain confidence enums (`high` · `moderate` · `low` · `speculative`) verbatim?
