...
(function (global) {
  ...
  function boot() {
    ...
    return Promise.all([sessionEngine.init(), portfolioEngine.init(), coachEngine.init()]).then(function () {
      ...
      try {
        if (String(global.location.hash || "").replace(/^#/, "") === "coach") {
          var panel = $("pfc-coach");
          var compare = $("pfa-compare-hint");
          setStatus(
            "Portfolio Coach lives here beside Portfolio Assistant — select two photographs to compare. Evidence only; you decide."
          );
          if (panel && panel.scrollIntoView) {
            panel.hidden = false;
            panel.scrollIntoView({ behavior: "smooth", block: "start" });
          } else if (compare && compare.scrollIntoView) {
