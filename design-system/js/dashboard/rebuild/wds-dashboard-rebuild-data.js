...
(function (global) {
  ...
  function hoursArtTransition(hours, cur) {
    var list = hours || [];
    var first = list[0] || {};
    var later = list[Math.min(3, Math.max(0, list.length - 1))] || first;
    var nowP =
      cur && cur.precipProb != null
        ? Number(cur.precipProb)
        : first.precipProb != null
          ? Number(first.precipProb)
          : 0;
    if (!isFinite(nowP)) nowP = 0;
    var laterP = later.precipProb != null ? Number(later.precipProb) : nowP;
    ...
    var cloud0 = /cloud|overcast|rain|storm/.test(sky0);
  ...
  function nextHoursPayload(platform) {
    ...
    return {
      ...
      current: cur,
      graphic: { kind: "hours", transition: hoursArtTransition(hours, cur) }
    };
  }
  ...
  function doorwayPayload(platform, now) {
    if (!platform) {
      return waitingOrUnavailable(
        null,
    ...
    if (Intel && typeof Intel.analyze === "function") {
      try {
        var analysis = Intel.analyze(platform, null, now || null);
  ...
  function buildWidgetPayload(id, platform, now) {
    ...
    if (id === "ph-doorway") return doorwayPayload(platform, now);
    return null;
  }
  ...
  function fromPlatform(platform, location, now) {
    var widgets = Object.create(null);
    LIVE_IDS.forEach(function (id) {
      widgets[id] = buildWidgetPayload(id, platform, now);
    });
    ...
    if (Intel && typeof Intel.analyze === "function" && platform) {
      try {
        intel = Intel.analyze(platform, location || null, now || null);
      } catch (e) {
        intel = null;
      }
    }
