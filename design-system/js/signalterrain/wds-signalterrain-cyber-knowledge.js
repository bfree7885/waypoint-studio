    ...
    return Promise.all([
      ...
      function body() {
        ...
        return (
          ...
          (state.teaching
            ? '<p class="st-badge">Teaching sample reference · shared graph · no offensive content</p>'
            : '<p class="st-badge">DATA STATUS: REAL graph · curated literacy indexes may be sparse · no offensive content</p>') +
          "<p>Move between today’s intelligence and lasting explanations — concepts, playbooks, incidents, and learning paths interconnected through the same subjects.</p>" +
          "<ul>" +
          "<li>" +
          ...
          (state.index.articles.length + state.index.playbooks.length + state.index.incidents.length + state.index.paths.length
            ? ""
            : "<p role=\"status\">NO CURRENT DATA for curated encyclopedia/playbook indexes in this deployment — live graph panels remain available.</p>") +
      ...
      function paint() {
        var modeBadge = state.teaching
          ...
          : '<p class="st-badge" role="status">DATA STATUS: REAL · SOURCE: data/cyber/graph.json · curated literacy indexes may be sparse · refreshed about every 6 hours.</p>';
        root.innerHTML =
