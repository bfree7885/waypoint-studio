...
(function (global) {
  ...
  function countryGraphCta(country, depth) {
    if (!country || !country.id) return "";
    var gl = graphLinks();
    var focusId = gl && gl.countryFocusId ? gl.countryFocusId(country.id) : country.id;
    if (country.id === "gsc_taiwan" || focusId === "gsc_taiwan") focusId = "gsn_taiwan";
    // detailDepth 0 = /countries/ ; 1 = /countries/<slug>/
    var base = gsRelativePrefix(depth != null ? depth : 1) + "relationship-graph/";
    if (gl && gl.ctaAnchor) {
      return (
        '<p class="gsc-detail__graph">' +
        gl.ctaAnchor(focusId, { base: base }) +
        "</p>"
      );
    }
    return (
      '<p class="gsc-detail__graph"><a class="gs-cta" href="' +
      base +
      "?focus=" +
    ...
    if (!country.citizenImpactConnections.length) {
      ...
      citizenHtml =
        ...
        country.citizenImpactConnections
          ...
          .join("") +
        "</ul>" +
        '<p class="gsc-note">Citizen Impact category links use stable section ids. Explore structural neighbors in the Relationship Graph.</p>';
    }
