...
assert.match(signal.url, /side-trails\/signalterrain\/?/);
...
assert.equal(
  nav.studioPrimaryNav.some((item) => item.id === "signalterrain"),
  false,
  "SignalTerrain must not appear in studioPrimaryNav"
);
assert.equal(
...
assert.match(about, /Waypoint Studio → Side Trails → SignalTerrain|Side Trails → SignalTerrain/);
