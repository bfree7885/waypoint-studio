...
async function staticChecks() {
  ...
  if (!/Side Trails/.test(html) || !/Waypoint Deck|waypoint-deck/.test(html)) fail("Side Trails / Deck missing");
  else pass("Side Trails Deck");
...
async function browserChecks() {
  ...
  for (const w of [375, 1440]) {
    ...
    const r = await send("Runtime.evaluate", {
      expression: `(() => {
        ...
        return {
          ...
          hasDeck: /Waypoint Deck/.test(text),
    ...
    if (!v.hasObserve || !v.hasDeck || !v.hasScenes || !v.hasSheds || !v.hasArticles) {
