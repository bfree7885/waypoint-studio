#!/usr/bin/env node
/**
 * Capture Dashboard RC3 Sprint 5 Personal Workspace screenshots (fixture hydrate — no live APIs required).
 * Usage: node automation/capture-dashboard-rc3-sprint5.mjs [baseUrl]
 */
...
const OUT = path.join(ROOT, "docs/rebuild-2026/dashboard-rc3-sprint5");
...
async function startChrome() {
  const userDataDir = fs.mkdtempSync(path.join(os.tmpdir(), "wdb-rc3s5-"));
  const proc = spawn(
    CHROME,
    [
      "--headless=new",
...
async function main() {
  ...
  console.log("done dashboard-rc3-sprint5");
}
