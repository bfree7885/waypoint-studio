#!/usr/bin/env node
/**
 * Capture Deep Forest Dispatch Batch #3 owner-review screenshots.
 * Usage: node automation/capture-dfd-batch3-screenshots.mjs [baseUrl]
 */
...
const CDP_PORT = Number(process.env.WAYPOINT_CDP_PORT || 9423);
const OUT = path.join(ROOT, "docs/deep-forest-dispatch/batch-3/screenshots");
const ART = "/opt/cursor/artifacts/dfd-batch3-review";

const STORIES = [
  {
    id: "kati-thanda-lake-eyre-fills",
    path: "/deep-forest-dispatch/stories/kati-thanda-lake-eyre-fills/",
    viz: "#two-faces"
  },
  {
    id: "channeled-scablands-floods",
    path: "/deep-forest-dispatch/stories/channeled-scablands-floods/",
    viz: "#oversized"
  },
  {
    id: "kgari-rainforest-on-sand",
    path: "/deep-forest-dispatch/stories/kgari-rainforest-on-sand/",
    viz: "#paradox"
  },
  {
    id: "columbia-glacier-satellite-retreat",
    path: "/deep-forest-dispatch/stories/columbia-glacier-satellite-retreat/",
    viz: "#proof"
  }
];
...
async function startChrome() {
  const userDataDir = fs.mkdtempSync(path.join(os.tmpdir(), "dfd-b3-shots-"));
  const proc = spawn(
    CHROME,
    [
      "--headless=new",
