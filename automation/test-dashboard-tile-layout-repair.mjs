...
assert("root redirects to canonical dashboard", /\/apps\/dashboard\//.test(rootHtml) && /location\.replace/.test(rootHtml));
assert("dashboard route mounts Rebuild CSS", /wds-dashboard-rebuild\.css/.test(dashHtml));
assert("dashboard mounts home-boot", /home-boot\.js/.test(dashHtml));
