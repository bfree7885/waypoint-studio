#!/usr/bin/env node
/**
 * Capture Dashboard Phase 2 screenshots (desktop + phone + customize/kiosk).
 * Usage: node automation/capture-dashboard-phase2.mjs [baseUrl]
 */
...
const CDP_PORT = Number(process.env.WAYPOINT_CDP_PORT || 9392);
const OUT = path.join(ROOT, "docs/rebuild-2026/phase2");
...
async function startChrome() {
  const userDataDir = fs.mkdtempSync(path.join(os.tmpdir(), "wdb-p2-"));
  const proc = spawn(
    CHROME,
    [
      "--headless=new",
...
async function main() {
  ...
  async function waitHydrated(maxMs = 20000) {
    ...
    while (Date.now() - start < maxMs) {
      ...
      if (state && state.hydrated && (state.facts >= 1 || /Air quality|Golden hour|°F|Moon/i.test(state.today))) {
        return state;
      }
      await delay(500);
    }
