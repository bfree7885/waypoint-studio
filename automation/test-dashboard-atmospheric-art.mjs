...
if (storm.includes("wdb-r-lightning") || storm.includes("108 34") || storm.includes("108 36")) {
  pass("storm has lightning");
} else fail("storm missing lightning");
if (rain.includes('stroke="#9eb8c8"') || rain.includes("wdb-r-rain")) pass("rain has streaks");
else fail("rain missing streaks");
if (snow.includes("translate(") || snow.includes("wdb-r-winter")) pass("snow has flakes");
else fail("snow missing flakes");

const aqi = Gfx.render({ kind: "aqi", value: 120 });
if (aqi.includes("wdb-r-depth") && (aqi.match(/<path/g) || []).length >= 2) {
  pass("Air valley/mountain with haze");
...
if (full !== crescent) pass("moon geometry differs full vs crescent");
else fail("moon full/crescent identical");

const alertQ = Gfx.render({ kind: "alert", active: false });
const alertA = Gfx.render({ kind: "alert", active: true, event: "Severe Thunderstorm Warning" });
...
if (
  (alertA.includes("wdb-r-lightning") || alertA.includes("108 34") || alertA.includes("108 36")) &&
  !alertQ.includes("wdb-r-lightning")
) {
  pass("lightning only when alert active");
...
if (
  ...
  (css.includes("0 0 22px") || css.includes("0 0 16px")) &&
  css.includes("--wdb-r-glow-strength")
) {
  pass("atmospheric luminous edge system preserved");
} else fail("neon edge system missing");
if (
  /customize-bar__columns[\s\S]*display:\s*none/.test(css) ||
  css.includes(".wdb-r-customize-bar__columns")
) {
  pass("mobile column rules still present");
...
if (geo28 && Math.abs(geo28.lit - 0.28) < 0.001 && geo28.waxing) {
  pass("illumination fraction drives lit amount");
} else fail("moonGeometry illumination not applied");
const crescentHtml = Gfx.render({
  kind: "moon",
  value: 28,
...
if (
  (crescentHtml.includes("#ebe6d8") || crescentHtml.includes("#e8e4d8")) &&
  crescentHtml.includes("wdb-r-luna")
) {
  pass("moon ivory-silver lit + faint dark limb");
