#!/usr/bin/env node
/**
 * Deck situational-awareness live engine
 *
...
} from "./lib/signal-engine.mjs";
import { enrichKevWithNvd } from "./lib/kev-nvd-enrich.mjs";
import { buildDashboardViews } from "./lib/dashboard-views.mjs";
import {
  buildAdaptiveDefense,
  ADAPTIVE_DEFENSE_VERSION
} from "./lib/adaptive-defense.mjs";
  ...
  const headers = Object.assign(
    {
      ...
      "User-Agent": "WaypointStudio-DeckSignals/1.0 (+https://waypointstudio.org; defensive-awareness)"
    },
    opts.headers || {}
  );
  try {
...
function recordsToGraphBundle(records) {
  ...
  return {
    meta: {
      ...
      status: "live",
      generatedAt: nowIso(),
      source: "deck-signals-live-engine",
...
async function main() {
  ...
  console.error(`Deck situational-awareness live engine ${ENGINE_VERSION} starting…`);
  ...
  const live = {
      ...
      dataState:
        trustState === "Live" || trustState === "Partial"
          ...
          : trustState === "Cached"
            ? "CACHED REAL"
            : "SOURCE UNAVAILABLE",
      engine: "deck-signals-live-engine",
    ...
    howToEvaluate: {
      title: "How this ingest evaluates cyber information",
      points: [
        ...
        "This ingest does not invent a composite threat level or world attack map without an authoritative source."
      ]
    }
  };
